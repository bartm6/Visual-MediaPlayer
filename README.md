...
The original media files are never modified. Browsing data such as thumbnails and timeline previews is stored separately in a reusable cache.
...
You can also open supported files through File Explorer with **Open with Visual MediaPlayer**. Opening another file reuses the existing application window; opening several files together creates a temporary mini-library containing those files.
...
Library scroll position is preserved when moving through Info/Player. Entering or leaving fullscreen preserves the **logical** scroll position as the number of cards per row changes; bottom stays bottom, top stays top, and intermediate positions remain approximately equivalent.
...
Timeline cards appear as their generated images become available. If generation was interrupted by entering Player, returning to Info resumes an incomplete timeline instead of leaving it permanently on “Loading Timeline”.
...
| `Esc` | Return to Info |
| `F11` | Toggle fullscreen |
| **180° / 360°** control | Change projection mode |
...
## Memory behavior during Load Everything

Load Everything is intentionally allowed to use more RAM than ordinary browsing. A single high-resolution/VR decode may legitimately use multiple GiB temporarily.

While Load Everything is active:

- that intentional decoder working set is not treated as normal Library memory pressure;
...
- only genuine machine-wide low-memory pressure is treated as a reason to hold back heavy batch work.

Timeline images generated for the currently open Info view are refreshed into the UI as they become available. If generation is interrupted by playback, returning to Info resumes an incomplete timeline.
...
In this source branch, the running `Uninstall.exe` cannot remove itself immediately. Windows therefore schedules the uninstaller and the now-empty installation directory for deletion on the next restart.
