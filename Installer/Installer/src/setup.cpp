...
bool ConfirmUninstall(bool& deleteCaches) {
    ...
    cfg.pszMainInstruction = L"Do you wish to uninstall Visual MediaPlayer?";
    ...
    const int answer = MessageBoxW(nullptr,
        L"Do you wish to uninstall Visual MediaPlayer?",
        L"Uninstall Visual MediaPlayer", MB_YESNO | MB_DEFBUTTON2);
    deleteCaches = false;
    return answer == IDYES;
}
...
void RemoveStartMenuEntriesAtRoot(const std::wstring& programsRoot) {
    ...
    // Current shortcut plus names/folders used by older builds. Restrict cleanup to
    // Visual MediaPlayer-owned entries under this Start Menu Programs directory.
    const wchar_t* shortcutNames[] = {
        L"Visual MediaPlayer.lnk",
        L"VisualMediaPlayer.lnk",
        L"Visual Media Player.lnk"
    };
    for (const wchar_t* name : shortcutNames)
        RemoveStartMenuFile(JoinPath(programsRoot, name));

    const wchar_t* legacyFolders[] = {
        L"Visual MediaPlayer",
        L"VisualMediaPlayer",
        L"Visual Media Player"
    };
    for (const wchar_t* name : legacyFolders) {
...
bool HasCommandLineArg(const wchar_t* wanted) {
...
DWORD CommandLineDwordValue(const wchar_t* prefix) {
    ...
    if (argv) {
        for (int i = 1; i < argc; ++i) {
            if (_wcsnicmp(argv[i], prefix, prefixLen) != 0) continue;
            const wchar_t* number = argv[i] + prefixLen;
            if (!*number) continue;
            wchar_t* end = nullptr;
            const unsigned long parsed = wcstoul(number, &end, 10);
...
void WaitForOriginalUninstaller() {
    const DWORD pid = CommandLineDwordValue(L"/wait-pid=");
...
bool RelaunchUninstallerFromTemp() {
    const std::wstring self = ModulePath();
    if (self.empty()) return false;

    wchar_t tempRoot[MAX_PATH + 1]{};
    const DWORD tempChars = GetTempPathW(static_cast<DWORD>(_countof(tempRoot)), tempRoot);
    if (!tempChars || tempChars >= _countof(tempRoot)) return false;

    const std::wstring helperName = L"VisualMediaPlayer-Uninstall-" +
        std::to_wstring(GetCurrentProcessId()) + L"-" +
        std::to_wstring(GetTickCount64()) + L".exe";
    const std::wstring helperPath = JoinPath(tempRoot, helperName);

    DeleteFileW(helperPath.c_str());
    if (!CopyFileW(self.c_str(), helperPath.c_str(), FALSE)) return false;

    std::wstring commandLine = L"\"" + helperPath + L"\" /uninstall /temp-uninstall /wait-pid=" +
    ...
    const BOOL launched = CreateProcessW(
        helperPath.c_str(), mutableCommand.data(), nullptr, nullptr, FALSE, 0, nullptr, tempRoot, &si, &pi);
    if (!launched) {
        DeleteFileW(helperPath.c_str());
        return false;
    }
...
bool Uninstall() {
    ...
    // The real uninstall runs from a temporary copy, so the complete application
    // directory can be removed immediately, including stale files from older builds.
    std::error_code installRemoveError;
    fs::remove_all(fs::path(installDir), installRemoveError);
    bool installDirRemoved = GetFileAttributesW(installDir.c_str()) == INVALID_FILE_ATTRIBUTES;
    if (!installDirRemoved) {
        // Best-effort immediate cleanup first; delayed deletion is only a last resort
        // for an unexpected external lock.
        DeleteFileW(JoinPath(installDir, kAppExe).c_str());
        DeleteFileW(JoinPath(installDir, kUninstallExe).c_str());
        RemoveDirectoryW(installDir.c_str());
        installDirRemoved = GetFileAttributesW(installDir.c_str()) == INVALID_FILE_ATTRIBUTES;
        if (!installDirRemoved)
            MoveFileExW(installDir.c_str(), nullptr, MOVEFILE_DELAY_UNTIL_REBOOT);
    }

    // Only the temporary helper itself may need delayed self-cleanup. It lives in
    // %TEMP%, not Program Files, and is not part of the installed application tree.
    const std::wstring self = ModulePath();
    if (!self.empty() && HasCommandLineArg(L"/temp-uninstall"))
        MoveFileExW(self.c_str(), nullptr, MOVEFILE_DELAY_UNTIL_REBOOT);

    MessageBoxW(nullptr,
        L"Visual MediaPlayer has been uninstalled.",
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) {
    ...
    bool ok = false;
    if (uninstall && !HasCommandLineArg(L"/temp-uninstall")) {
        // Stage the uninstaller in %TEMP% so C:\Program Files\Visual MediaPlayer can
        // be removed immediately rather than waiting for a reboot.
        ok = RelaunchUninstallerFromTemp();
        if (!ok) ok = Uninstall();
    } else {
        if (uninstall && HasCommandLineArg(L"/temp-uninstall")) WaitForOriginalUninstaller();
