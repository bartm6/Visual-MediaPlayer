...
private:
        ...
        case WM_MOUSEWHEEL:
            if (mode_ == Mode::Library) {
                ...
                if ((GET_KEYSTATE_WPARAM(w) & MK_CONTROL) != 0) {
                    ...
                    ClampScroll(); InvalidateLibraryScrollableArea();
                }
    ...
    float LibraryReturnHighlightAmount(size_t mediaIndex) const {
        ...
        // Three smooth light pulses across the 3-second return-focus period.
        // This uses the exact same border treatment as hover; only its intensity
        // breathes so the returned media is easy to locate without altering the image.
        const float t=static_cast<float>(elapsed)/static_cast<float>(kLibraryReturnHighlightDurationMs);
        const float wave=std::sin(3.0f*PI_F*t);
        return EaseUi(wave*wave);
    }
    ...
    void DrawBitmapCenteredTintWhite(HDC dc, RECT r, Gdiplus::Bitmap* bmp, int insetX = 8, int insetY = 8) {
        if (!bmp || bmp->GetLastStatus() != Gdiplus::Ok) return;
        const float availW = static_cast<float>(std::max(1L, (r.right - r.left) - insetX * 2));
        const float availH = static_cast<float>(std::max(1L, (r.bottom - r.top) - insetY * 2));
        const float bw = static_cast<float>(bmp->GetWidth());
        ...
        const Gdiplus::ColorMatrix whiteMatrix = {
        ...
        attrs.SetColorMatrix(&whiteMatrix, Gdiplus::ColorMatrixFlagsDefault, Gdiplus::ColorAdjustTypeBitmap);
        g.DrawImage(bmp, Gdiplus::RectF(dx, dy, dw, dh), 0.0f, 0.0f, bw, bh, Gdiplus::UnitPixel, &attrs);
    }
    ...
    void CleanupOrphanMediaCache(const std::vector<fs::path>& discoveredCacheRoots) {
        ...
        auto remember=[&](const MediaItem& item) {
            ...
            if (!item.uiCachePath.empty()) {
                ...
                if(item.isVideo) expectedThumbFiles.insert(CacheCleanupPathKey(fs::path(BannerTimestampPath(item.uiCachePath))));
            }
    ...
    void StartThumbnailWorker() {
        StopThumbnailWorker();
        if(mode_==Mode::Player || fullLoadRunning_.load(std::memory_order_acquire)) return;
            ...
            for(const auto& job:jobs) {
                ...
                if(sourceReady && WaitForGenerationClaim(job.source,fullLoadStop_)) {
                    ...
                    if(job.isVideo) {
                        ...
                        jobOk=libraryReady && infoReady && std::abs(infoTime-libraryTime)<=0.001 && PreviewCacheFilesCompleteForDir(job.previewDir);
                    } else {
    ...
    void StartLibraryThumbLoader() {
        ...
        auto worker=[this]() {
            ...
            for(;;){
                ...
                // All filesystem access and JPEG decoding for Library cards happens here,
                // never in WM_PAINT / the mouse-wheel path.
                if(CacheFileLooksHealthy(job.cachePath,512)){
                    ...
                    else if(!thumbWorkerRunning_.load(std::memory_order_acquire)) result.privateDecodeFailed=true;
                }

                // Keep the existing cached Windows thumbnail as a temporary fallback while
                // our deterministic private thumbnail is still being generated. VR waits for
                // the private cache so packed stereo frames never flash in the Library.
                if(!result.bitmap && !(job.isVideo && job.isVr))
    ...
    void RefreshLibraryThumbViewport(RECT rc) {
        ...
        const bool changed=libraryThumbViewportScrollY_!=scrollY_ ||
        ...
        if(libraryThumbViewportScrollY_>=0 && scrollY_!=libraryThumbViewportScrollY_)
    ...
    HBITMAP GetLibraryItemThumb(MediaItem& item, size_t index, int w, int h) {
        const ULONGLONG now=GetTickCount64();
        item.thumbLastUsed=now;
        const bool privateLargeEnough=item.thumb && item.thumbFromPrivateCache && item.thumbW>=w && item.thumbH>=h;
        if(privateLargeEnough) return item.thumb;
        ...
        if(!alreadyRequested && now>=item.thumbNextLoadAttempt){
            ...
            {
                std::lock_guard<std::mutex> lock(libraryThumbLoadMutex_);
                libraryThumbLoadJobs_.push_back(std::move(job));
            }
    ...
    void ApplyLibraryThumbLoadResults() {
        ...
        for(auto& result:ready){
            ...
            if(item.thumbLoadRequestEpoch==result.epoch) item.thumbLoadRequestEpoch=0;
    ...
    void QueueDetailPrefetchWindow() {
        ...
        std::vector<DetailPrefetchJob> jobs; jobs.reserve(indices.size());
        for(size_t idx:indices){
            if(idx>=items.size()) continue;
            const auto& item=items[idx];
            DetailPrefetchJob job;
            job.generation=generation; job.category=category_; job.index=idx; job.mediaPath=item.path;
            job.bannerPath=item.isVideo?item.cachePath:item.path; job.isVideo=item.isVideo;
            const bool current=(idx==selected_);
            const bool bannerFileReady=item.isVideo ? CacheFileLooksHealthy(job.bannerPath,1024) : FileHasData(fs::path(job.bannerPath));
            job.loadBanner=!current && item.detailThumb==nullptr && bannerFileReady;
            job.previewDir=item.isVideo?BuildPreviewDirectory(item.path):L"";
            job.loadPreviews=item.isVideo && !current && prefetchedPreviewSets_.find(item.path)==prefetchedPreviewSets_.end();
    ...
    void PaintLibraryNavigator(HDC dc, RECT rc) {
        ...
        // Library maintenance controls stay pinned to the bottom-right. Folder and
        // rescan use compact native glyphs; Load everything remains text-labelled.
        const int rightMargin=18, iconW=46, loadW=142, gap=10;
        const int right=std::max(0,static_cast<int>(rc.right)-rightMargin);
        rescanRect_={right-iconW,buttonTop,right,buttonBottom};
        chooseRect_={rescanRect_.left-gap-iconW,buttonTop,rescanRect_.left-gap,buttonBottom};
        loadEverythingRect_={chooseRect_.left-gap-loadW,buttonTop,chooseRect_.left-gap,buttonBottom};
        DrawButton(dc,loadEverythingRect_,L"Load everything");
    ...
    void PaintLibrary(HDC dc, RECT rc) {
        ...
        for(size_t displayIndex=firstDisplay;displayIndex<lastDisplay;++displayIndex){
            ...
            HBITMAP bmp=GetLibraryItemThumb(mutableList[i],i,640,360);
    ...
    void HandleClick(int x,int y) {
        ...
        if(mode_==Mode::Library){
            ...
            if(PtInRect(&videosTabRect_,p)){
                ...
                SaveCurrentFolderViewState(); SaveSettings(); ClampScroll(); InvalidateRect(hwnd_,nullptr,FALSE); return;
            }
            ...
            if(PtInRect(&loadEverythingRect_,p)){ StopImageSlideshow(); StartFullLoadEverything(); return; }
    ...
    bool FindAnimatedButtonRect(HWND owner, POINT p, RECT& out) const {
        ...
        if(mode_==Mode::Library){
            ...
            if(hit(loadEverythingRect_)) return true;
    ...
    void Layout() {
        ...
        // Center the time readout on the Play/Pause button, in the free strip above it.
        ...
        const int volumeRight=playerAutoNextRect_.left-18,volumeLeft=std::max(static_cast<int>(playerSkipForwardRect_.right)+18,volumeRight-190);
    ...
    std::unique_ptr<Gdiplus::Bitmap> folderIconBitmap_,refreshIconBitmap_,skipBackIconBitmap_,skipForwardIconBitmap_;
    RECT chooseRect_{},rescanRect_{},loadEverythingRect_{},videosTabRect_{},imagesTabRect_{},slideshowRect_{},imageDetailsSlideshowRect_{},backRect_{},playRect_{},searchBoxRect_{},detailsFooterRect_{},previewZoomRect_{},detailsPrevRect_{},detailsNextRect_{};
    ...
    RECT playerBackRect_{},playerVrToggleRect_{},playerSkipBackRect_{},playerPlayRect_{},playerSkipForwardRect_{},playerFullRect_{},playerAutoNextRect_{},seekRect_{},volumeRect_{},volumeLabelRect_{},playerTimeRect_{};
    ...
    Category libraryThumbViewportCategory_=Category::Videos; std::wstring libraryThumbViewportFolder_;
