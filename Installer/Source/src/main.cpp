    ...
    int Run() {
        ...
        while (true) {
            while (PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE)) {
                ...
                // Space is an app-level media shortcut. Handle it before dispatch so it
                // works even when focus is temporarily on one of VMP's child/popup windows.
                // Ignore auto-repeat so holding Space cannot start playback and then
                // immediately pause it again on the next repeated key event.
                if (msg.message == WM_KEYDOWN && msg.wParam == VK_SPACE && (msg.lParam & 0x40000000) == 0) {
...
private:
    ...
    void StartPreviewWorkerForSelected() {
        ...
        if (category_!=Category::Videos||selected_>=videos_.size()) { ClearLoadingState(); return; }
