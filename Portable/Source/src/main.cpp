...
private:
    ...
    int DetectPackedStereoFromCurrentFrame() {
        ...
        auto measureLR = [&]() -> Metric {
            ...
            const int maxShift=std::clamp(static_cast<int>(halfW/45u),2,6);
            Metric best{};
            constexpr int sxCount=14,syCount=10;
        ...
        auto measureTB = [&]() -> Metric {
            ...
            const int maxShift=std::clamp(static_cast<int>(probeW/90u),2,6);
            Metric best{};
            constexpr int sxCount=14,syCount=10;
            for(int shift=-maxShift;shift<=maxShift;++shift){
                double a=0,b=0,aa=0,bb=0,ab=0,mad=0; int count=0;
        ...
        auto likely=[](const Metric& m){
            if(m.contrast<7.0) return false;
            return (m.corr>=0.78&&m.mad<=58.0)||(m.corr>=0.66&&m.mad<=34.0);
        };
        ...
        if(lr.contrast<7.0 && tb.contrast<7.0) return -1; // fade/black/flat frame: retry
        return 0; // sufficiently detailed frame, but not a convincing stereo pair
    }
...
private:
    ...
    std::wstring BuildCachePath(const std::wstring& source) const {
        std::wstring sig=L"detail-vr-crop-v4-audited-layout|"+source;
        std::error_code ec;
    ...
    std::wstring BuildUiCachePath(const std::wstring& source) const {
        std::wstring sig=L"grid-v6-vr-crop-audited-640|"+source;
        std::error_code ec;
        auto sz=fs::file_size(source,ec); if(!ec) sig+=L"|"+std::to_wstring(sz);
        ec.clear(); auto ft=fs::last_write_time(source,ec); if(!ec) sig+=L"|"+std::to_wstring(ft.time_since_epoch().count());
        const uint64_t hash=Fnv1a64(sig);
    ...
    std::wstring BuildPreviewDirectory(const std::wstring& source) const {
        std::wstring sig=L"details-previews-v4-layout-lock|"+source;
        std::error_code ec;
        auto sz=fs::file_size(source,ec); if(!ec) sig+=L"|"+std::to_wstring(sz);
        ec.clear(); auto ft=fs::last_write_time(source,ec); if(!ec) sig+=L"|"+std::to_wstring(ft.time_since_epoch().count());
        const uint64_t hash=Fnv1a64(sig);
    ...
    static StereoMetric MeasureStereoLR(Gdiplus::Bitmap& bitmap) {
        ...
        const int maxShift=std::clamp(static_cast<int>(half/90u),2,14);
        StereoMetric best{};
        constexpr int samplesX=14, samplesY=10;
    ...
    static StereoMetric MeasureStereoTB(Gdiplus::Bitmap& bitmap) {
        ...
        const int maxShift=std::clamp(static_cast<int>(w/180u),2,14);
        StereoMetric best{};
        constexpr int samplesX=14, samplesY=10;
    ...
    static bool LikelyStereo(const StereoMetric& m) {
        // Low-contrast frames are deliberately treated as inconclusive/mono instead of
        // guessing stereo. The layout is locked once per video, preventing frame-to-frame flips.
        if (m.contrast < 7.0) return false;
        return (m.corr >= 0.78 && m.mad <= 58.0) ||
               (m.corr >= 0.66 && m.mad <= 34.0);
    }
    ...
    static bool GenerateVideoPreviewsMF(const std::wstring& source, const std::wstring& previewDir, VRInfo vr, std::atomic<bool>& stop, HWND notifyHwnd, std::atomic<double>* durationOut = nullptr) {
        ...
        auto finalizePendingLayout=[&](){
            if(layoutState<0){
                ...
                else if(layoutVotes[1]>0 && layoutVotes[2]==0 && layoutVotes[0]==0) layoutState=1;
                else if(layoutVotes[2]>0 && layoutVotes[1]==0 && layoutVotes[0]==0) layoutState=2;
        ...
        for (int sec : captureSeconds) {
            ...
            if(layoutState<0){
                ...
                if(layoutVotes[1]>=2 || layoutVotes[2]>=2 || pending.size()>=3) finalizePendingLayout();
            } else {
    ...
    bool PreviewCacheIsComplete() const {
        ...
        for(int sec:expected){
            wchar_t name[32]{}; swprintf_s(name,L"%06d.jpg",sec);
            if(!FileHasData(fs::path(previewDir_)/name)) return false;
        }
        return true;
    }
    ...
    void StartPreviewWorkerForSelected() {
        ...
        previewThread_=std::thread([this,item,dir=previewDir_,previewsComplete]() {
            ...
            if(!previewStop_.load()&&!fs::exists(item.cachePath)) {
                ...
                if (GenerateVideoCache(high)) HideCacheRootIfCreated(item.path);
                if(hwnd_&&!previewStop_.load()) PostMessageW(hwnd_,WM_APP_THUMB_READY,0,0);
            }
            if(!previewStop_.load()&&!previewsComplete) {
                if (GenerateVideoPreviewsMF(item.path,dir,item.vr,previewStop_,hwnd_,&detailsDurationSeconds_)) HideCacheRootIfCreated(item.path);
            }
    ...
    static bool GenerateVideoStillMF(const std::wstring& source, const std::wstring& output, VRInfo vr,
                                     int targetW, int targetH, ULONG quality) {
        ComPtr<IMFAttributes> attrs;
        if (FAILED(MFCreateAttributes(&attrs,2))) return false;
        attrs->SetUINT32(MF_SOURCE_READER_ENABLE_ADVANCED_VIDEO_PROCESSING,TRUE);
        attrs->SetUINT32(MF_READWRITE_DISABLE_CONVERTERS,FALSE);
        ...
        const double scale=std::min(1.0,std::min(static_cast<double>(targetW)/nativeW,static_cast<double>(targetH)/nativeH));
        UINT outW=std::max<UINT>(2u,static_cast<UINT>(nativeW*scale));
        UINT outH=std::max<UINT>(2u,static_cast<UINT>(nativeH*scale));
        outW=(outW+1u)&~1u; outH=(outH+1u)&~1u;
        ...
        const double seekSeconds=duration>0.2?std::clamp(duration*0.25,0.0,std::max(0.0,duration-0.1)):0.0;

        auto readAt=[&](double seconds)->ComPtr<IMFSample>{
        ...
        ComPtr<IMFSample> chosen=readAt(seekSeconds);
        ...
        if(layoutState<0){
            ...
            if(duration>0.4){
                for(double fraction : {0.50,0.75}){
                    ...
                    ComPtr<IMFSample> probe=readAt(probeSeconds);
                    if(probe) vote(probe.Get());
                }
            }
            if(votes[1]>=2) layoutState=1;
            else if(votes[2]>=2) layoutState=2;
            else if(votes[1]>0 && votes[2]==0 && votes[0]==0) layoutState=1;
            else if(votes[2]>0 && votes[1]==0 && votes[0]==0) layoutState=2;
        ...
        if(dirEc) return false;
        return SaveVideoSampleJpeg(chosen.Get(),actualW,actualH,stride,vr,layoutState,output,targetW,targetH,quality,true);
    }

    static bool GenerateVideoCache(const ThumbJob& job) {
        if(fs::exists(job.output)) return true;
        const std::wstring tmp=job.output+L".tmp.jpg"; DeleteFileW(tmp.c_str());
        if(!GenerateVideoStillMF(job.source,tmp,job.vr,2560,1440,95)){ DeleteFileW(tmp.c_str()); return false; }
    ...
    static bool GenerateGridThumb(const ThumbJob& job) {
        ...
        bool ok=false;
        if (job.isVideo) {
            // A normal video may reuse an already-existing Windows thumbnail. VR media
            // always uses our MF path so the stereo layout is decided consistently and
            // a raw packed/double-eye Shell thumbnail can never leak into the UI.
            if(!job.vr.vr){
            ...
            if(!ok) ok=GenerateVideoStillMF(job.source,tmp,job.vr,640,360,92);
        } else {
    ...
    void PaintDetails(HDC dc, RECT rc) {
        ...
        const int edgeCenterY=(64+footerTop)/2;
        const int edgeTop=std::max(68,edgeCenterY-edgeH/2);
    ...
    void NavigateDetailsMedia(int direction) {
        ...
        StopImageSlideshow();
        if(category_==Category::Videos){ StopPreviewWorker(); ClearPreviewBitmaps(); }
    ...
    bool comInitialized_=false,mfStarted_=false; ULONG_PTR gdiplusToken_=0; std::thread thumbThread_; std::atomic<bool> thumbStop_{false};
