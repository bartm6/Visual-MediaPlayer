...
public:
    ...
    int DetailsPreviewCardWidthForViewport(int clientWidth) const {
        if (UseTenAcrossFullscreenPreviews(clientWidth)) return TenAcrossFullscreenPreviewWidth(clientWidth);
        if (!fullscreen_ && !previewZoomOverridden_) return SevenAcrossWindowedPreviewWidth(clientWidth);
...
private:
    ...
    LRESULT HandleMain(UINT m, WPARAM w, LPARAM l) {
        ...
        case WM_MOUSEMOVE:
            ...
            if (mode_ == Mode::Library && libraryScrollDragging_) {
                ...
                ClearMediaHoverImmediate();
                DeferBackgroundWork();
                InvalidateLibraryScrollWithFooter();
                return 0;
            }
        ...
        case WM_MOUSEWHEEL:
            if (mode_ == Mode::Library) {
                ...
                if ((GET_KEYSTATE_WPARAM(w) & MK_CONTROL) != 0) {
                    ...
                    ClampScroll(); InvalidateLibraryScrollWithFooter();
                }
                ...
                if (category_ == Category::Images && ctrlDown && !nativeImageSizing_) {
                ...
                if (category_ == Category::Videos && ctrlDown) {
                    ...
                    if (wheelSteps != 0) {
                        ...
                        // Windowed Details defaults to seven previews across. The first
                        // Ctrl+wheel gesture takes that computed width as its starting point,
                        // then switches to the normal freely scalable preview width.
    ...
    void InvalidateLibraryScrollWithFooter() {
        if(!hwnd_) return;
        RECT rc{}; GetClientRect(hwnd_,&rc);
        // The footer is alpha-composited over Library content. During scroll/zoom it
        // must be repainted with the moving content or it looks like a frozen snapshot.
    ...
    void QueuePriorityResolutionMetadataForSearch() {
        ...
        for(auto& item:videos_){
            ...
            if(ToLower(fs::path(item.path).parent_path().lexically_normal().wstring())!=currentKey) continue;
    ...
    void PaintLibraryNavigator(HDC dc, RECT rc) {
        ...
        const bool showChooseOnly=!showRootMaintenance && currentFolder_.empty();
    ...
    void HandleClick(int x,int y) {
        ...
        if(mode_==Mode::Library){
            ...
            if((IsAtChosenLibraryRoot() || currentFolder_.empty()) && PtInRect(&chooseRect_,p)){ StopImageSlideshow(); ChooseFolder(); return; }
    ...
    bool IsLibraryVolumePresent() const {
        ...
        const std::wstring& path = !persistentFolder_.empty() ? persistentFolder_ : folder_;
    ...
    void CheckLibraryAccessHealth() {
        ...
        if(libraryUnavailableLatched_){
            ...
            if(IsLibraryRootAccessible()){
                ...
                libraryAccessRetryNeedsRescan_=false;
                ClearInAppNotice();
                if(mode_==Mode::Library) Scan(false);
            }
            return;
        }
    ...
    bool FindAnimatedButtonRect(HWND owner, POINT p, RECT& out) const {
        ...
        if(mode_==Mode::Library){
            ...
            if((IsAtChosenLibraryRoot() || currentFolder_.empty()) && hit(chooseRect_)) return true;
    ...
    void UpdateMediaHover(int x,int y) {
        ...
        if(!found) nextSurface=MediaHoverSurface::None;

        if(nextSurface==mediaHoverSurface_ && nextId==mediaHoverId_ && SameRect(nextRect,mediaHoverRect_)) return;
    ...
    bool FindAdjacentDetailsMedia(size_t current, int direction, size_t& target) const {
        ...
        return FindAdjacentInSameFolder(CurrentItems(),current,direction,target);
    }
