...
public:
    ...
    bool FlatZoomActive() const { return !vrInfo_.vr && !nativePixelSizing_ && std::abs(flatZoom_-1.0f) > 0.001f; }
    ...
    void FlatWheelZoom(short delta, int x, int y) {
        ...
        // Below fit-to-window the full frame is already visible, so keep it centered
        // while allowing the user to shrink it down to 25% of the fitted size.
        if(newZoom<1.0f){
            flatZoom_=newZoom; flatCenterU_=0.5f; flatCenterV_=0.5f;
            return;
        }
    ...
    void BeginDrag(int x, int y) {
        ...
        if (!vrInfo_.vr && (nativePixelSizing_ || flatZoom_ <= 1.001f)) return;
    ...
    void Drag(int x, int y) {
        ...
        sx=std::max(0.0001f,sx); sy=std::max(0.0001f,sy);
        const float zoom=std::max(1.001f,flatZoom_);
...
private:
    ...
    void DrawLibraryGpuVideoBadges(MediaItem& item,RECT card) {
        ...
        if(klass){
            ...
            else {src=resolution4kBitmap_.get();cache=&libraryD2dResolution4k_;}
    ...
    void PaintLibraryGpuLoadingPopup(RECT rc) {
        ...
        const int width=300,height=58,right=std::max(20,static_cast<int>(rc.right)-18),top=searchVisible_?68:18;RECT box{std::max(8,right-width),top,right,top+height};
    ...
    void EnforceProcessMemoryBudget() {
        ...
        if(processBytes>=kProcessMemoryPanicRelease){
            // Last-resort cache purge. During playback preserve only the small return
            // working set so Back -> Library still has its visible rows ready.
            ResetLibraryThumbLoadView();
            if(mode_==Mode::Player && !playbackLibraryWarmPaths_.empty()){
                auto purgeCold=[&](std::vector<MediaItem>& list){
                    for(auto& item:list){
                        if(playbackLibraryWarmPaths_.find(item.path)!=playbackLibraryWarmPaths_.end()) continue;
    ...
    void TrimLibraryGpuTextures() {
        ...
        if(processBytes>=kProcessMemoryEmergency){for(auto& e:entries) if(!e.visible) release(e);}
    ...
    bool PaintLibraryGpu(RECT rc) {
        ...
        const uint64_t processBytes=ProcessMemoryBytes();
        int prefetchRows=10;if(processBytes>=kProcessMemoryEmergency)prefetchRows=0;else if(processBytes>=kProcessMemoryHighPressure)prefetchRows=2;else if(processBytes>=kNormalProcessMemoryTarget)prefetchRows=5;
    ...
    bool WaitForBackgroundPermit(const std::atomic<bool>& stop) const {
        while (!stop.load(std::memory_order_acquire)) {
            // Do not start another source decode/generation while the process is near its
            // internal allocation guard. The UI thread is responsible for cache trimming;
            // background producers resume automatically after pressure has fallen.
            if(ProcessMemoryBytes()>=kProcessMemoryAllocationGuard){ Sleep(20); continue; }
    ...
    void TrimPreviewMemory() {
        ...
        while(loaded.size()>keep){PreviewFrame* frame=loaded.front();loaded.erase(loaded.begin());if(frame->bitmap){DeleteObject(frame->bitmap);frame->bitmap=nullptr;}}
    ...
    void StartLibraryThumbLoader() {
        ...
        auto worker=[this]() {
            ...
            for(;;){
                ...
                if(memoryBefore>=kProcessMemoryPanicRelease) continue;
                if(memoryBefore>=kProcessMemoryEmergency && !job.allowGenerate) continue;
                const bool allowSourceWork=memoryBefore<kProcessMemoryAllocationGuard;
    ...
    void PrimeVisibleLibraryThumbsFromPrivateCache() {
        if(mode_!=Mode::Library || !hwnd_ || ProcessMemoryBytes()>=kProcessMemoryEmergency) return;
    ...
    HBITMAP GetLibraryItemThumb(MediaItem& item, size_t index, int w, int h, bool highPriority=false) {
        ...
        if(!highPriority && ProcessMemoryBytes()>=kProcessMemoryEmergency) return nullptr;
    ...
    void ApplyLibraryThumbLoadResults() {
        ...
        for(auto& result:ready){
            ...
            if(result.bitmap){
                ...
                if(ProcessMemoryBytes()>=kProcessMemoryAllocationGuard && !protectedNow){DeleteObject(result.bitmap);result.bitmap=nullptr;item.thumbNextLoadAttempt=now+1500;continue;}
    ...
    void ClearThumbs(std::vector<MediaItem>& list) {
        for(auto& v:list){ if(v.thumb) DeleteObject(v.thumb); if(v.detailThumb) DeleteObject(v.detailThumb); v.thumb=nullptr; v.detailThumb=nullptr; v.libraryGpuThumb.Reset(); v.libraryGpuThumbSource=nullptr; v.libraryGpuGeneration=0; v.thumbW=v.thumbH=0; v.detailThumbW=v.detailThumbH=0; v.thumbLoadRequestEpoch=0; v.thumbNextLoadAttempt=0; }
    }
    ...
    void TrimThumbMemory() {
        ...
        const bool forceEviction=processBytes>=kProcessMemoryAllocationGuard;
        ...
        for(const auto& ref:loaded){
            ...
            // The playback return set is intentionally tiny (viewport + two rows), so
            // preserve it even under high pressure. Ordinary viewport protection may be
            // broken only by the 1.4 GiB emergency cache purge.
            if(keepPlaybackReturn || (keepCurrentViewport&&!forceEviction)) continue;
    ...
    void ToggleFullscreen() {
        ...
        Layout();
        if(mode_==Mode::Library) PrimeVisibleLibraryThumbsFromPrivateCache();
    ...
    ComPtr<ID2D1Bitmap> libraryD2dFavoriteIcon_,libraryD2dVrIcon_,libraryD2dResolution4k_,libraryD2dResolution5k_,libraryD2dResolution8k_,libraryD2dFolderIcon_,libraryD2dRefreshIcon_,libraryD2dDownloadIcon_;
    uint64_t libraryD2dGeneration_=1; int libraryD2dWidth_=0,libraryD2dHeight_=0;
