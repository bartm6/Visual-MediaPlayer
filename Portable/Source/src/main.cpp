...
public:
    ...
    bool FlatZoomActive() const { return !vrInfo_.vr && !nativePixelSizing_ && flatZoom_ > 1.001f; }
    ...
    void FlatWheelZoom(short delta, int x, int y) {
        ...
        const float oldZoom=std::max(1.0f,flatZoom_);
        const float factor=std::pow(1.15f,static_cast<float>(delta)/120.0f);
        const float newZoom=std::clamp(oldZoom*factor,1.0f,8.0f);
        if(newZoom<=1.001f){ ResetFlatZoom(); return; }
        ...
        const float halfU=0.5f/(sx*newZoom);
        const float halfV=0.5f/(sy*newZoom);
        newCenterU=halfU>=0.5f?0.5f:std::clamp(newCenterU,halfU,1.0f-halfU);
        newCenterV=halfV>=0.5f?0.5f:std::clamp(newCenterV,halfV,1.0f-halfV);
    ...
    void BeginDrag(int x, int y) {
        // VR keeps its existing mouse-look path. Flat media only enters drag mode
        // while zoomed and Native Size is off, so normal video clicks are unchanged.
        if (!vrInfo_.vr && (nativePixelSizing_ || !FlatZoomActive())) return;
    ...
    void Drag(int x, int y) {
        ...
        if (nativePixelSizing_ || flatZoom_ <= 1.001f || !videoWindow_ || !eyeW_ || !eyeH_) return;
        RECT rc{}; GetClientRect(videoWindow_,&rc);
        const float clientW=static_cast<float>(std::max<LONG>(1L,rc.right-rc.left));
        const float clientH=static_cast<float>(std::max<LONG>(1L,rc.bottom-rc.top));
        const float viewportAspect=clientW/clientH;
        ...
        sx=std::max(0.0001f,sx); sy=std::max(0.0001f,sy);
        const float zoom=std::max(1.0f,flatZoom_);
        ...
        const float halfU=0.5f/(sx*zoom);
        const float halfV=0.5f/(sy*zoom);
        flatCenterU_=halfU>=0.5f?0.5f:std::clamp(flatCenterU_,halfU,1.0f-halfU);
        flatCenterV_=halfV>=0.5f?0.5f:std::clamp(flatCenterV_,halfV,1.0f-halfV);
    }
    void EndDrag() {
        if (!dragging_) return;
        dragging_ = false;
...
private:
    ...
    void DrawVideoSurface(ID3D11ShaderResourceView* srv,const SurfaceState& s,float opacity) {
        ...
        if(s.nativePixelSizing && !s.vr.vr){
            ...
            const float fit=std::min(1.0f,std::min(clientW/sourceW,clientH/sourceH));
            renderW=std::max(1.0f,std::floor(sourceW*fit+0.5f));
            renderH=std::max(1.0f,std::floor(sourceH*fit+0.5f));
...
public:
    ...
    int LibraryWheelPixelsPerNotch() const {
        const int imageH=std::max(113,static_cast<int>(std::lround(static_cast<double>(libraryCardWidth_)*9.0/16.0)));
        const int rowStride=imageH+kLibraryTitleHeight+kLibraryGap;
        const int defaultImageH=std::max(113,static_cast<int>(std::lround(static_cast<double>(kDefaultLibraryCardWidth)*9.0/16.0)));
        const int defaultStride=defaultImageH+kLibraryTitleHeight+kLibraryGap;
        // Keep the familiar windowed scroll rate, but scale it with the actual row
        // height so large/fullscreen cards do not feel slower simply because each row
        // contains more vertical pixels.
...
private:
    ...
    LRESULT HandleMain(UINT m, WPARAM w, LPARAM l) {
        ...
        case WM_MOUSEWHEEL:
            ...
            return 0;
        case WM_LBUTTONDOWN:
            if (mode_ == Mode::Details && category_ == Category::Images && !nativeImageSizing_ && ImageZoomActive()) {
        ...
        case WM_TIMER:
            ...
            if (w == kResumeDetailsWorkersTimerId) {
                ...
                if(mode_==Mode::Details && category_==Category::Videos){
                    ...
                    if(externalMediaSession_ && !PreviewCacheIsComplete()) {
                        // External/Open-With playback may enter Player before any Info cache
                        // existed. Once playback releases the source, build the missing
                        // banner/timeline in the normal cache location. Existing cache is
                        // restored immediately and therefore does not get duplicated.
                        StartPreviewWorkerForSelected();
                    }
                    ClampDetailsScroll();
                    QueueDetailPrefetchWindow();
            ...
            if (w == kLiveWindowMoveTimerId) {
                ...
                if(mode_==Mode::Player && player_){
                    ...
                    // Render from this timer to keep video advancing during that loop.
                    Layout();
                    player_->Render();
                    UpdateSeekUi();
                    UpdatePlayerControlVisibility();
            ...
            break;
        case WM_KEYDOWN:
            if (mode_ == Mode::Library && (GetKeyState(VK_CONTROL) & 0x8000)) {
                if (w == L'F') {
                    searchVisible_ = true;
                    searchSelectAll_ = !searchQuery_.empty();
                    InvalidateRect(hwnd_, nullptr, FALSE);
                    return 0;
                }
                if (w == L'A' && searchVisible_ && !searchQuery_.empty()) {
    ...
    static void DeletePreviewFrameBitmaps(std::vector<PreviewFrame>& frames) {
        for(auto& p:frames){ if(p.bitmap) DeleteObject(p.bitmap); p.bitmap=nullptr; }
    }
    ...
    void TrimPreviewMemory() {
        // Keep timeline frames that the user has already viewed resident for the lifetime
        // of the Info session. They are released when the active/parked detail window is
        // trimmed or Info is closed, rather than reread from the VeraCrypt volume.
    }
    ...
    void StartFullLoadEverything() {
        ...
        fullLoadThread_=std::thread([this,jobs=std::move(jobs)]() mutable {
            ...
            for(const auto& job:jobs) {
                ...
                if(!WaitForBackgroundPermit(fullLoadStop_)) break;
    ...
    void StartResolutionMetadataWorker() {
        ...
        resolutionMetadataThread_=std::thread([this](){
            ...
            for(;;){
                ...
                if(!job.highPriority && !WaitForBackgroundPermit(resolutionMetadataStop_)) break;
    ...
    static int ResolutionBadgeClass(const MediaItem& item) {
        ...
        struct Candidate { UINT px; int klass; };
        constexpr Candidate candidates[]={{3840u,4},{5120u,5},{7680u,8}};
        const Candidate* best=nullptr;
        UINT bestDiff=0xFFFFFFFFu;
        for(const auto& c:candidates){
            const UINT diff=span>c.px?span-c.px:c.px-span;
            if(diff<bestDiff){bestDiff=diff;best=&c;}
        }
        if(!best) return 0;
        // Accept nearby DCI/VR/encode dimensions, but do not label ordinary HD/QHD.
        if(static_cast<double>(bestDiff)/static_cast<double>(best->px)>0.16) return 0;
        return best->klass;
    }
    ...
    void StartLibraryThumbLoader() {
        ...
        auto worker=[this]() {
            ...
            for(;;){
                ...
                if(job.loadBitmap && !result.bitmap && !cacheReady && job.allowGenerate &&
                ...
                if(job.loadBitmap && job.allowGenerate && !result.bitmap && !(job.isVideo && job.isVr))
        ...
        // Four on-demand workers reduce the time visible Library banners spend as
        // placeholders. They still generate only currently requested/visible media, so
        // this increases foreground throughput without reintroducing whole-library preload.
        for(int i=0;i<4;++i) libraryThumbLoadThreads_.emplace_back(worker);
    }
    ...
    void RefreshLibraryThumbViewport(RECT rc) {
        ...
        // A scroll changes priority, not media identity. Keep in-flight thumbnail reads
        // so a wheel tick cannot discard a bitmap after the drive was already touched.
        if(identityChanged){
            libraryThumbViewEpoch_.fetch_add(1,std::memory_order_acq_rel);
            std::lock_guard<std::mutex> lock(libraryThumbLoadMutex_);
            libraryThumbLoadJobs_.clear();
    ...
    void InvalidateLibraryScrollWithFooter(int oldScrollY) {
        if(!hwnd_) return;
        RECT rc{}; GetClientRect(hwnd_,&rc);
        const int pixelDelta=oldScrollY-scrollY_;
        // Keep popups/search, scrollbar and footer fixed while the large central media
        // surface is shifted. Narrow edge/top strips are cheap to repaint even at 4K.
        const int fixedTop=std::min(96,std::max(0,static_cast<int>(rc.bottom)-64));
        const int fixedSide=std::min(72,std::max(0,static_cast<int>(rc.right)/4));
        RECT moving{fixedSide,fixedTop,std::max<LONG>(fixedSide,rc.right-fixedSide),std::max<LONG>(fixedTop,rc.bottom-64)};
        if(!ScrollBufferedRegionVertical(pixelDelta,moving)){
            InvalidateRect(hwnd_,&rc,FALSE);
            return;
        }
        RECT top{0,0,rc.right,fixedTop};
        RECT left{0,fixedTop,fixedSide,std::max<LONG>(fixedTop,rc.bottom-64)};
        RECT right{std::max<LONG>(0,rc.right-fixedSide),fixedTop,rc.right,std::max<LONG>(fixedTop,rc.bottom-64)};
        RECT footer{0,std::max<LONG>(0,rc.bottom-64),rc.right,rc.bottom};
        InvalidateRect(hwnd_,&top,FALSE);
        InvalidateRect(hwnd_,&left,FALSE);
        InvalidateRect(hwnd_,&right,FALSE);
        // The footer is alpha-composited over the moving Library content.  It must
        // be recomposited after every scroll or the pixels visible through it become
        // a frozen snapshot of the frame on which the footer was first painted.
    ...
    void InvalidateDetailsScrollOptimized(int oldScrollY) {
        if(!hwnd_) return;
        RECT rc{}; GetClientRect(hwnd_,&rc);
        const int pixelDelta=oldScrollY-detailsScrollY_;
        const int fixedTop=std::min(96,std::max(0,static_cast<int>(rc.bottom)-64));
        const int fixedSide=std::min(72,std::max(0,static_cast<int>(rc.right)/4));
        RECT moving{fixedSide,fixedTop,std::max<LONG>(fixedSide,rc.right-fixedSide),std::max<LONG>(fixedTop,rc.bottom-64)};
        if(!ScrollBufferedRegionVertical(pixelDelta,moving)){
        ...
        InvalidateRect(hwnd_,&left,FALSE);
        InvalidateRect(hwnd_,&right,FALSE);
        // Info uses the same translucent footer.  Repaint this thin strip against
        // the newly scrolled hero/timeline content so the transparency remains live.
        InvalidateRect(hwnd_,&footer,FALSE);
    }
    ...
    static void DrawBitmapContainNoUpscale(HDC dc,HBITMAP bmp,RECT r) {
        if(!bmp) return; BITMAP bm{}; GetObjectW(bmp,sizeof(bm),&bm); if(!bm.bmWidth||!bm.bmHeight) return;
        HDC mem=CreateCompatibleDC(dc); HGDIOBJ old=SelectObject(mem,bmp); SetStretchBltMode(dc,HALFTONE);
        const double fit=std::min(static_cast<double>(r.right-r.left)/bm.bmWidth,static_cast<double>(r.bottom-r.top)/bm.bmHeight);
        const double scale=std::min(1.0, fit);
    ...
    static void DrawBitmapContainZoom(HDC dc,HBITMAP bmp,RECT r,float zoom,float centerU,float centerV) {
        ...
        const double scale=fit*std::max(1.0f,zoom);
        const int dw=std::max(1,static_cast<int>(std::lround(bm.bmWidth*scale))),dh=std::max(1,static_cast<int>(std::lround(bm.bmHeight*scale)));
    ...
    void ClearThumbs(std::vector<MediaItem>& list) {
        for(auto& v:list){ if(v.thumb) DeleteObject(v.thumb); if(v.detailThumb) DeleteObject(v.detailThumb); v.thumb=nullptr; v.detailThumb=nullptr; v.thumbW=v.thumbH=0; v.detailThumbW=v.detailThumbH=0; v.thumbLoadRequestEpoch=0; v.thumbNextLoadAttempt=0; }
    }

    void TrimThumbMemory() {
        constexpr size_t kLibraryThumbSoftLimit=300;
        ...
        if(loaded.size()<=kLibraryThumbSoftLimit) return;

        // 300 is a soft target, not a hard viewport ceiling. Never evict a thumbnail
        // that belongs to the currently visible rows or the small prefetch zone.
        // On an unusually large/tall viewport RAM may temporarily exceed 300 images;
        // once those cards leave the active zone, normal LRU trimming can remove them.
        std::sort(loaded.begin(),loaded.end(),[](const Ref&a,const Ref&b){return a.tick<b.tick;});
        size_t remaining=loaded.size();
        for(const auto& ref:loaded){
            if(remaining<=kLibraryThumbSoftLimit) break;
            auto* v=ref.item;
            if(protectedLibraryThumbPaths_.find(v->path)!=protectedLibraryThumbPaths_.end()) continue;
            if(v->thumb){
                DeleteObject(v->thumb); v->thumb=nullptr; v->thumbW=v->thumbH=0;
                v->thumbAttempted=false; v->thumbFromPrivateCache=false;
        ...
        for(size_t i=0;i<list.size();++i){
            ...
            if(!filters.text.empty()){
                ...
                while(start<filters.text.size()){
                    ...
                    if(!word.empty() && item.searchText.find(word)==std::wstring::npos){textMatch=false;break;}
    ...
    void PaintLibrary(HDC dc, RECT rc) {
        ...
        for(size_t displayIndex=firstDisplay;displayIndex<lastDisplay;++displayIndex){
            ...
            if(mutableList[i].isVideo){
                ...
                if(Gdiplus::Bitmap* resolutionIcon=ResolutionBadgeBitmap(mutableList[i])){
                    RECT resolutionTag{badgeRight-28,card.top+8,badgeRight,card.top+36};
                    DrawBitmapCentered(dc,resolutionTag,resolutionIcon,0,0);
                }
            }
        ...
        // Quietly prefetch two rows outside the viewport. Prioritize the direction of
        // travel, then the opposite side. These active rows are protected from LRU
        // eviction; 300 remains the normal soft RAM target for everything else.
        ...
        if(media.bottom>0 && media.top<footerTop){
            ...
            if(bmp){
                if(item.isVideo) {
                    ...
                    if(previous){ if(nativeImageSizing_) DrawBitmapContainNoUpscale(dc,previous,media); else DrawBitmapContain(dc,previous,media); }
                    if(nativeImageSizing_) DrawBitmapContainAlphaNoUpscale(dc,bmp,media,static_cast<BYTE>(std::clamp<int>(static_cast<int>(std::lround(progress*255.0f)),0,255)));
                    ...
                    if(nativeImageSizing_) DrawBitmapContainNoUpscale(dc,bmp,media);
                ...
                if(item.isVideo){
                    ...
                    if(Gdiplus::Bitmap* resolutionIcon=ResolutionBadgeBitmap(item)){
    ...
    void EnterPlayerAt(double startSeconds) {
        ...
        mode_=Mode::Player; if(!CreatePlayerControls()){DestroyPlayerFooterTransition();mode_=Mode::Details;StartPreviewWorkerForSelected();QueueDetailPrefetchWindow();InvalidateRect(hwnd_,nullptr,TRUE);return;}
        ApplyMainWindowCornerPreference();
    ...
    bool ImageZoomActive() const { return imageZoomScale_>1.001f; }
    void ResetImageZoom() {
    ...
    void ZoomImageAtPoint(short delta, POINT p) {
        ...
        const float oldZoom=std::max(1.0f,imageZoomScale_);
        const float factor=std::pow(1.15f,static_cast<float>(delta)/120.0f);
        const float newZoom=std::clamp(oldZoom*factor,1.0f,8.0f);
        if(newZoom<=1.001f){ ResetImageZoom(); InvalidateRect(hwnd_,nullptr,FALSE); return; }
    ...
    void PanImageZoomByDelta(int dx,int dy) {
        if(!imageZoomDragging_ || nativeImageSizing_ || !ImageZoomActive() || mode_!=Mode::Details || category_!=Category::Images || selected_>=images_.size()) return;
        auto& item=images_[selected_];
        UINT sourceW=item.detailThumbW>0?static_cast<UINT>(item.detailThumbW):item.sourceWidth;
        UINT sourceH=item.detailThumbH>0?static_cast<UINT>(item.detailThumbH):item.sourceHeight;
        if(!sourceW || !sourceH){ if(!GetCurrentImageNativeSize(sourceW,sourceH)) return; }
    ...
    void ApplyNativeVideoWindowSize() {
        ...
        const int monitorW=std::max(1,static_cast<int>(mi.rcMonitor.right-mi.rcMonitor.left));
        const int monitorH=std::max(1,static_cast<int>(mi.rcMonitor.bottom-mi.rcMonitor.top));
        // Native means one source pixel maps to one client pixel whenever the source
        // fits the physical monitor. Do not shrink 4K merely to make room for the
        // title bar/taskbar; that was the small windowed/fullscreen mismatch.
        const double fit=std::min(1.0,std::min(static_cast<double>(monitorW)/static_cast<double>(sourceSize.first),
                                               static_cast<double>(monitorH)/static_cast<double>(sourceSize.second)));
        const int targetClientW=std::max(1,static_cast<int>(std::lround(static_cast<double>(sourceSize.first)*fit)));
        const int targetClientH=std::max(1,static_cast<int>(std::lround(static_cast<double>(sourceSize.second)*fit)));
    ...
    void ApplyNativeImageWindowSize() {
        ...
        const int monitorW=std::max(1,static_cast<int>(mi.rcMonitor.right-mi.rcMonitor.left));
        const int monitorH=std::max(1,static_cast<int>(mi.rcMonitor.bottom-mi.rcMonitor.top));
        const double fit=std::min(1.0,std::min(static_cast<double>(monitorW)/static_cast<double>(sourceW),
                                               static_cast<double>(monitorH)/static_cast<double>(sourceH)));
        const int targetClientW=std::max(1,static_cast<int>(std::lround(static_cast<double>(sourceW)*fit)));
        const int targetClientH=std::max(1,static_cast<int>(std::lround(static_cast<double>(sourceH)*fit)));
        const int targetWindowW=targetClientW+nonClientW;
        const int targetWindowH=targetClientH+nonClientH;
        int x=mi.rcMonitor.left+(monitorW-targetWindowW)/2;
        int y=mi.rcMonitor.top+(monitorH-targetWindowH)/2;
    ...
    void ToggleFullscreen() {
        ...
        Layout();
        if(mode_!=Mode::Player) InvalidateRect(hwnd_,nullptr,TRUE);
    ...
    std::unique_ptr<Gdiplus::Bitmap> folderIconBitmap_,refreshIconBitmap_,skipBackIconBitmap_,skipForwardIconBitmap_,downloadIconBitmap_,resolution4kBitmap_,resolution5kBitmap_,resolution8kBitmap_,vrBadgeBitmap_,vrBadgeWhiteBitmap_,resizeIconBitmap_,resizeIconWhiteBitmap_;
    ...
    std::set<std::wstring> protectedLibraryThumbPaths_;
